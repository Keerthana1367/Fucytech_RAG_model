...
def ingest_markdown_reports(reports_path) -> list[Document]:
    """Ingest professional Markdown knowledge documents."""
    docs = []
    path = Path(reports_path)
    if not path.exists():
        return docs
    ...
    for md_file in sorted(path.glob("*.md")):
        ...
        # Split by level 2 headers (##) to create manageable chunks
        sections = re.split(r"\n(?=## )", content)
...
def ingest_pdfs(pdf_dir) -> list[Document]:
    """Ingest PDF files by extracting text and chunking."""
    docs = []
    path = Path(pdf_dir)
    if not path.exists():
        return docs
    ...
    for pdf_file in sorted(path.glob("*.pdf")):
        ...
        try:
            ...
            # 규정 (Regulation) documents are dense; 2000 chars is usually enough for context.
            chunk_size = 2000
            overlap = 200
...
def load_all_documents() -> list[Document]:
    ...
    kb_docs = ingest_markdown_reports(REPORTS_PATH)
